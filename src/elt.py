...
def create_loan_contract(df_freddie_o, df_freddie_p, df_fannie_o, df_fannie_p):
    ...
    def missing_checker_channel(df):
        df_fannie_o_temp.filter((df_fannie_o_temp["channel"] == "") | df_fannie_o_temp["channel"].isNull() | isnan(
            df_fannie_o_temp["channel"])).count()
    ...
    def avg_rate_us_by_year(df):
        avg_rate_us_by_year = df.groupBy("originate_year").agg(F.mean("original_interest_rate"),
                                                               F.count("original_interest_rate"))

    avg_rate_us_by_year = df_loans_save.groupBy("originate_year").agg(F.mean("original_interest_rate"), \
                                                                      F.count("original_interest_rate")).orderBy(
        'originate_year', ascending=True)
